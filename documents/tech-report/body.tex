...
To take an explicit example, after 27 roll-out steps (162 hours) IFS ENS (mean) {\it vs.} Analysis RMSE as recorded by Weatherbench 2 was 1.699 K. The equivalent for IFS ENS (1st member) {\it vs.} Analysis was 2.554 K. The discrepancy between Dawn and Baskerville at this point was 0.0937 K.
...
\begin{table}[t]
    ...
    \begin{tabular}{ll|rrrr}
        \parbox{1cm}{System \\} & \parbox{1cm}{Mode \\} & \parbox{2.3cm}{\raggedleft Mean time-step /s} &  \parbox{2.2 cm}{\raggedleft Time for first step /s} & \parbox{2.2cm}{\raggedleft Time for 28 steps /s} & \parbox{2.1cm}{\raggedleft Total run \\ time /s} \\ \hline
...
In terms of GPU energy usage, the lowest-energy configuration is one node with one GPU total on Baskerville (55\,267.94 J) and one node with one GPU total on Dawn (332\,511.66 J). The highest total energy occurs at two nodes with eight GPUs total on Baskerville (181\,773.66 J) and four nodes with four GPUs total on Dawn (1\,355\,921.92 J).

For average power draw per GPU, the lowest values are at two nodes with eight GPUs total on Baskerville (84.55 W) and four nodes with four GPUs total on Dawn (287.52 W), while the highest values occur at one node with one GPU total on both systems (209.15 W on Baskerville; 386.30 W on Dawn).

\begin{table}[t]
    \centering
    \hbadness=99999
    \begin{tabular}{rr|rr}
        \parbox{1cm}{Nodes \\} & \parbox{1cm}{GPUs \\} & \parbox{4.3cm}{\raggedleft Avg energy used per training run /J}& \parbox{3.2cm}{\raggedleft Avg power draw per GPU /W} \\ \hline
        1 & 1 & 55\,267.94 & 209.15 \\
        1 & 4 & 118\,982.26 & 98.17 \\
        2 & 4 & 86\,393.69 & 120.83 \\
        4 & 4 & 76\,523.08 & 138.63 \\
        2 & 8 & 181\,773.66 & 84.55 \\
    \end{tabular}
...
\begin{table}[t]
    ...
    \begin{tabular}{rr|rr|r}
        \parbox{1cm}{Nodes \\} & \parbox{1cm}{GPUs \\} & \parbox{4.3cm}{\raggedleft Avg energy used per training run /J}& \parbox{3.2cm}{\raggedleft Avg power draw per GPU /W} & \parbox{3.0cm}{\raggedleft Energy ratio \\ \,}\\ \hline
        1 & 1 & 332\,511.66 & 386.30 & 6.02 \\
        1 & 4 & 473\,221.48 & 364.58 & 3.98 \\
        2 & 4 & 761\,781.29 & 319.00 & 8.82 \\
        4 & 4 & 1\,355\,921.92 & 287.52 & 17.72 \\
        2 & 8 & 598\,843.44 & 327.95 & 3.29 \\
    \end{tabular}
...
\begin{table}[t]
    ...
    \hbadness=99999
    \begin{tabular}{rrrr}
         \parbox{3cm}{File Size (MiB) \\ \,} & \parbox{3cm}{Cached (Y/N) \\ \,} & \parbox{3.5cm}{Dawn Bandwidth (GiB/s)} &  \parbox{3.3cm}{Bask Bandwidth (GiB/s)} \\ \hline
           423 & N & 0.5 & 1.7 \\
   423 & Y & 7 & 5.5 \\
    \end{tabular}
...
On Dawn, with device hierarchy set to \textit{composite}, we found that loading this dataset from disk (Lustre) to RAM took 25.6 seconds, roughly equivalent to 3.4 GiB / 25.6 s = 0.13 GiB s\textsuperscript{-1}. The average time to move each batch of training data from RAM to XPU was 0.0002 seconds, giving a total time of 0.0002 $\times$ 28 = 0.0056 s for all batches. This is roughly equivalent to 3.4 GiB / 0.0056 s = 607 GiB s\textsuperscript{-1} (assuming the full dataset is used during training).

On Baskerville, we found that loading this dataset from disk to RAM took 37.3 seconds, roughly equivalent to 3.4 GiB / 37.3 seconds = 0.09 GiB s\textsuperscript{-1}. The average time to move each batch of training data from RAM to GPU was 0.082 s, giving a total time of 0.082 $\times$ 28 = 2.296 s for all batches. This is roughly equivalent to 3.4 GiB / 2.296 s = 1.48 GiB s\textsuperscript{-1} (assuming the full dataset is used during training).
