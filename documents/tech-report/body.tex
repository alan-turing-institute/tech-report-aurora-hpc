...
\begin{enumerate}
    ...
    \item The CPU to GPU/XPU bandwidth on Dawn is consistently higher than on Baskerville by a factor of up to $\times$\,4.4.
...
Intel\textsuperscript{\tiny\textregistered} Data Center Max 1550 GPUs can be used in two modes, called {\it hierarchies}. In the {\it flat\/} hierarchy, each {\it stack} is exposed as a separate GPU and addressable as if it were a different piece of hardware. In this mode, there appear to be eight addressable GPUs on the node (2 {\it stacks} $\times$ 4 GPU cards). In the {\it composite} hierarchy, both {\it stacks} are treated as one root device composed of multiple sub-devices. Implicit scaling then means that the two stacks appear as a single GPU.
...
Baskerville offers NVIDIA A100 GPUs; we used the 80 GiB variants of these to ensure we had enough memory to perform fine-tuning of Aurora. Each node has four such GPUs and 2 $\times$ Intel\textsuperscript{\tiny\textregistered} Xeon\textsuperscript{\tiny\textregistered} Platinum 8360Y CPUs, giving a total of 72 cores at 2.4 GHz (with boost to 3.5 GHz) and 512 GiB RAM (16 $\times$ 32 GiB DDR4).

Baskerville GPUs provide a typical arrangement for running AI workflows, similar to the \href{https://learn.microsoft.com/en-us/azure/virtual-machines/sizes/gpu-accelerated/nca100v4-series}{NC\_A100\_v4 series} of virtual machines available on Azure.
...
We downloaded 9 days' worth of data at 6-hour time-steps (36 time-steps total) from 2023-01-01 to 2023-01-09 and at times of 00:00, 06:00, 12:00 and 18:00. This dataset took up around 3.6 GB (3.63 GiB) on disk as netCDF files. The variables provided are the following:
...
Table \ref{tab:inference} summarises these results.

\begin{table}[!ht]
    \centering
    \hbadness=99999
    \begin{tabular}{ll|rrrr}
        \parbox{1cm}{System \\} & \parbox{1cm}{Mode \\} & \parbox{2.2cm}{\raggedleft Mean time step /s} &  \parbox{2.2 cm}{\raggedleft Time for first step /s} & \parbox{2.2cm}{\raggedleft Time for 28 steps /s} & \parbox{2.2cm}{\raggedleft Total run \\ time /s} \\ \hline
...
On Dawn and Baskerville, we ran multi-GPU fine-tuning with un-sharded FSDP (equivalent to using DDP) using 1 node (1 and 4 GPUs total), 2 nodes (4 and 8 GPUs total) and 4 nodes (4 GPUs total). We did this using the PyTorch FSDP class and setting the FSDP sharding strategy to \texttt{NO\_SHARD}.

For these runs, we used a batch size of 1 ({\it i.e.} one aurora \texttt{Batch}), the PyTorch {\tt DistributedSampler}, AdamW optimizer and our Aurora loss function for fine-tuning. To compare our different runs, we updated our model gradients only once per 8 batches of data. This was to allow for the fact that when training with DDP and 8 GPUs, 8 batches of data would pass through our model (in 8 parallel threads) before our model gradients would be updated. Thus, when running with 1 GPU, our model gradients should also only be updated after every 8 batches of data. An iteration is therefore one forward pass (using an Aurora \texttt{Batch} with two time points to predict one future time point) and one backwards pass and, every eight iterations, we then do one optimiser step.
...
Due to access restrictions, we were not able to capture processor utilisation on Dawn. Instead, we use GPU power consumption captured using {\tt xpu-smi}\ as a proxy for this. During the most active periods power consumption of the Intel GPUs reached 1600 Watts (the maximum) on all eight devices (using \textit{composite} device hierarchy). Memory utilisation consistently reached above 60\% with a maximum of 66.82\% ({\it i.e.\/} 85.5 GiB per GPU).

\subsubsection{Energy usage comparison}
\label{section:energy-comparison}


For the same training exercise as in Tables \ref{tab:fine-tune-Dawn} and \ref{tab:fine-tune-bask}, we captured GPU energy usage from {\tt xpu-smi}\ on Dawn and {\tt nvidia-smi}\ on Baskerville. We ran 4 repeats of the training and calculated the average energy used per training run by dividing by four. These energies include overheads such as model and data loading, process initialisation, as well as the energy to train the model - this choice was made to reflect the true energy requirements for running this entire workload. Tables \ref{tab:energy-bask} and \ref{tab:energy-dawn} show the average GPU energy per training run and average power draw per GPU for each configuration, with the Dawn-to-Baskerville energy ratio included alongside the Dawn results. These figures report GPU energy only and do not include CPU, memory, networking, or system-level overheads.

Overall, Dawn consumed between $3.3\times$ and $17.7\times$ more GPU energy than Baskerville for the same training exercise. This is driven by both higher mean power draw (roughly $1.9\times$--$3.9\times$ higher) and, in most configurations, longer elapsed time (roughly $0.85\times$--$3.26\times$).

In terms of GPU energy usage, the lowest-energy configuration is 1 node with 1 GPU total on Baskerville (55,267.94 J) and 1 node with 1 GPU total on Dawn (332,511.66 J). The highest total energy occurs at 2 nodes with 8 GPUs total on Baskerville (181,775.24 J) and 4 nodes with 4 GPUs total on Dawn (1,355,923.71 J).

For average power draw per GPU, the lowest values are at 2 nodes with 8 GPUs total on Baskerville (84.55 W) and 4 nodes with 4 GPUs total on Dawn (287.52 W), while the highest values occur at 1 node with 1 GPU total on both systems (209.15 W on Baskerville; 386.30 W on Dawn).
...
\begin{table}[!ht]
    ...
    \hbadness=99999
    \begin{tabular}{rr|rr}
        \parbox{1cm}{Nodes \\} & \parbox{1cm}{GPUs \\} & \parbox{2.3cm}{\raggedleft Avg energy used per training run /J}& \parbox{2.2cm}{\raggedleft Avg power draw per GPU /W} \\ \hline
        1 & 1 & 55,267.94 & 209.15 \\
        1 & 4 & 118,982.26 & 98.17 \\
        2 & 4 & 86,398.03 & 120.84 \\
        4 & 4 & 76,522.08 & 138.63 \\
        2 & 8 & 181,775.24 & 84.55 \\
    \end{tabular}
...
\begin{table}[!ht]
    ...
    \begin{tabular}{rr|rr|r}
        \parbox{1cm}{Nodes \\} & \parbox{1cm}{GPUs \\} & \parbox{2.3cm}{\raggedleft Average energy used per training run /J}& \parbox{2.2cm}{\raggedleft Avg power draw per GPU /W} & \parbox{2.0cm}{\raggedleft Energy ratio}\\ \hline
        1 & 1 & 332,511.66 & 386.30 & 6.01 \\
        1 & 4 & 473,221.48 & 364.58 & 3.98 \\
        2 & 4 & 761,781.29 & 319.00 & 8.82 \\
        4 & 4 & 1,355,923.71 & 287.52 & 17.72 \\
        2 & 8 & 598,530.15 & 327.78 & 3.29 \\
    \end{tabular}
...
On Dawn, with device hierarchy set to \textit{composite}, we found that loading this dataset from disk (Lustre) to RAM took 25.6 seconds -- roughly equivalent to 3.4 GB / 25.6 s = 0.13 GB s\textsuperscript{-1}. The average time to move each batch of training data from RAM to XPU was 0.0002 seconds, giving a total time of 0.0002 $\times$ 28 = 0.0056 s for all batches. This is roughly equivalent to 3.4 GB / 0.0056 s = 607 GB s\textsuperscript{-1} (assuming the full dataset is used during training).

On Baskerville, we found that loading this dataset from disk to RAM took 37.3 s -- roughly equivalent to 3.4 GB / 37.3 s = 0.09 GB s\textsuperscript{-1}. The average time to move each batch of training data from RAM to GPU was 0.082 s, giving a total time of 0.082 $\times$ 28 = 2.296 s for all batches. This is roughly equivalent to 3.4 GB / 2.296 s = 1.48 GB s\textsuperscript{-1} (assuming the full dataset is used during training). 

These timings suggest data transfer both from disk to RAM and from RAM to GPU/XPU are faster on Dawn, in agreement with the results above. Notably, however, RAM to XPU transfer times seem to be approximately 410 $\times$ quicker on Dawn versus Baskerville -- this may suggest an error in our code or calculation of RAM to XPU transfer time as this seems exceedingly fast.
...
It's clear that the Dawn team are keen for users to get the most from the system and realise that, given users are less likely to be familiar with an Intel-based GPU system, they may require additional support in deploying workloads.
...
The ranges for the random numbers were selected carefully so that the element-wise values of the result matrices would roughly double every $s = 128$ steps.
...
The results for the two runs when running in composite were identical. Similarly the results for the two runs when running in flat were identical. However, in the \fpdouble\ case, the output from the XPU when running in Composite mode was very marginally different from the output when running in Flat mode, as can be seen in Table \ref{tab:flatresultsfirst}. It's not clear why this might be happening, but we note this is not an example of non-deterministic output, but rather differences due to different configurations.
